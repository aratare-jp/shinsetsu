...
(defresolver bookmark-tag-resolver-by-bookmark
  [{{user-id :user/id} :request} {:bookmark/keys [id] :as input}]
  {::pc/input  #{:bookmark/id}
   ::pc/output [{:bookmark/tags [:tag/id]}]}
  (if-let [err (m/explain s/bookmark-tag-fetch-by-bookmark-spec (merge input {:bookmark/user-id user-id}))]
    ...
    (do
      ...
      (let [input {:bookmark-tag/bookmark-id id :bookmark-tag/user-id user-id}
            tags  (bookmark-tag-db/fetch-tags-by-bookmark input)]
