...
(defexpect tab-db
  (let [user       (g/generate User default-leaf-generator)
        ...
        tab        (merge (g/generate Tab default-leaf-generator)
                          {:tab/user-id user-id})
        tab-id     (:tab/id tab)
    ...
    (tab-compare tab (read-tab tab-id))
    (tab-compare new-tab (update-tab new-tab))
    (tab-compare new-tab (read-tab tab-id))
    (tab-compare new-tab (delete-tab tab-id))
    (expect nil? (read-tab tab-id))))

(comment
  (require '[eftest.runner :as ef])
  (ef/run-tests [#'shinsetsu.db.tab-test/tab-db]))
